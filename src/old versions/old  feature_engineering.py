...
def calculate_rfm_features(conn, lookback_days=365, reference_date=None):
    ...
    WITH CustomerTransactions AS (
        ...
        WHERE 
            d.type = 'OWNER'
            AND t.Trans_date >= DATEADD(day, -{lookback_days}, {date_clause})
            AND t.Trans_date <= '{reference_date.strftime('%Y-%m-%d')}'  -- Ensure transactions are before or on reference date
        GROUP BY 
            d.client_id
    )
    SELECT 
        ...
        total_monetary_value / transaction_frequency AS monetary_per_transaction
    FROM 
...
def extract_product_portfolio_features(conn):
    ...
    LEFT JOIN 
        CreditCard c ON d.disp_id = c.disp_id
    LEFT JOIN 
...
def generate_comprehensive_features(conn, reference_date=None, lookback_days=365):
    """
    Generate a comprehensive set of features for all customers directly from the database
    ...
    Returns:
        pandas.DataFrame: Comprehensive feature set for all customers
    """
    # Extract
    ...
    rfm_features = calculate_rfm_features(conn, lookback_days, reference_date)
    product_features = extract_product_portfolio_features(conn)
    transaction_features = extract_transaction_behavior_features(conn, lookback_days, reference_date)
    ...
    try:
            ...
            SELECT 
                district_id,
                A3 AS population,
                A4 AS num_municipalities,
                A9 AS entrepreneurs_per_1000,
                A10 AS committed_crimes,
                A11 AS crimes_1995,
                A12 AS crimes_1996,
                A13 AS crimes_1997
            FROM 
    ...
    comprehensive_features = combine_features(
            rfm_features, 
            product_features, 
...
def get_complete_customer_features_from_dataframes(transaction_data, customer_data, loan_data=None, 
                                                card_data=None, order_data=None, demographic_data=None, 
                                                reference_date=None, lookback_days=None):
    """
    Generate a complete set of customer features for CLV and segmentation using dataframes
    
    Args:
        transaction_data: DataFrame with transaction data
        customer_data: DataFrame with customer data
        loan_data: DataFrame with loan data (optional)
        card_data: DataFrame with credit card data (optional)
        order_data: DataFrame with permanent order data (optional)
        demographic_data: DataFrame with demographic data (optional)
        reference_date: Reference date for recency calculation (optional)
        lookback_days: Number of days to look back for transaction patterns (optional)
        
    Returns:
        pandas.DataFrame: Complete feature set for all customers
    """
    # Calculate RFM features
    rfm_features = calculate_rfm_features_from_dataframes(transaction_data, customer_data, reference_date)
...
def main():
    """
    Example usage of the feature engineering functions
    """
    try:
        ...
        # Use a fixed reference date from the past that's more likely to include your historical data
        # Adjust this date as needed based on when your data is from
        reference_date = datetime(1998, 12, 30)
        print(f"Using reference date: {reference_date}")

        # Generate comprehensive features with a longer lookback period
        print("Generating comprehensive features...")
        comprehensive_features = generate_comprehensive_features(
            conn, 
            reference_date=reference_date,
            lookback_days=3650  # 10 years - adjust as needed
        )
...
try:
    ...
    test_query = "SELECT TOP 1 * FROM Trans"
    test_result = pd.read_sql(test_query, conn)
    print("Database connection successful")
    
    # Check Loan table structure
    loan_query = "SELECT TOP 1 * FROM Loan"
    loan_result = pd.read_sql(loan_query, conn)
    print("Loan table columns:", loan_result.columns.tolist())
    
    # Check other critical tables
    tables = ["Account", "Client", "Disposition", "Card", "PermanentOrder"]
    for table in tables:
        try:
